...
/**
 ...
 void homewizard_Kwh_loop() {
    ...
    const auto currents = getEVFromHomeWizardKwh();
#if SMARTEVSE_VERSION < 40 //v3
    for (int i = 0; i < currents.first; i++)
        EVMeter.Irms[i] = currents.second[i];
    if (currents.first) {
        CalcIsum();
        EVMeter.setTimeout(COMM_TIMEOUT);
    }
#else
    Serial1.printf("@Irms:%03u,%d,%d,%d\n", EVMeter.Address, currents.second[0], currents.second[1], currents.second[2]); //Irms:011,312,123,124 means: the meter on address 11(dec) has Irms[0] 312 dA, Irms[1] of 123 dA, Irms[2] of 124 dA
#endif
}
